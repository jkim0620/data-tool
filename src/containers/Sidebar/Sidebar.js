        ...
        <div className="tool-wrapper__sidebar">            
            ...
            {selectedView === 'bubble' ?
                <div className="tool-wrapper__sidebar--desc">
                    {selectedIndustry === 'Athletic Apparel' && <p>The overall imbalance favoring <span className="font-text-bold emerald">Demand-side</span> functions exceeds 2:1 for brands producing “soft goods” where detailed documentation of the <span className="font-text-bold yolk">manufacturing</span> process suffers in favor of product use and sustainability of apparel and footwear with a short, limited lifespan.</p>}
                    {selectedIndustry === 'Automotive' && <p>Among the minority of industries favoring <span className="font-text-bold yolk">Supply-side</span> functions, detailed documentation about <span className="font-text-bold yolk">manufacturing</span> extends to the <span className="font-text-bold yolk">movement</span> of OEM vehicles through a well-established, 3-tier supply chain.</p>}
                    ...
                    {selectedIndustry === 'Chemicals & Materials' && <p>Chemical & Material brands register the highest share of documentation in the <span className="font-text-bold emerald">regenerative</span> function, reflecting the need to adhere to regulations governing the disclosure of industrial processes that are potentially harmful to human health and the environment.</p>}
                    {selectedIndustry === 'Consumer Packaged Goods' && <p>In contrast to adjacent industries such as Beauty, CPG brands inspire more documentation around the <span className="font-text-bold yolk">manufacturing</span> process, driven by a need to demonstrate the health and safety of products used with greater frequency by more members of the immediate family.</p>}
                    {selectedIndustry === 'Food & Beverage' && <p>Among the most “balanced” industries across competing functions (with one notable exception), Food & Beverage reflects a wider reliance on indirect distribution channels and wholesalers to <span className="font-text-bold emerald">sell</span> perishable product to consumers.</p>}
                    ...
                    {selectedIndustry === 'Industrial Equipment' && <p>In contrast to Automotive brands, Industrial Equipment manufacturers are focused on extending fleet life via <span className="font-text-bold emerald">regenerative</span> mechanisms (repair/refurbish) for product that undergoes fewer changes over longer refresh cycles.</p>}
                    {selectedIndustry === 'Multinationals' && <p>Representing some of the most complex and diversified product assortments (including Aerospace & Defense), Multinational brands have a similar signature to CPG, producing mass documentation on <span className="yolk font-text-bold">manufacturing</span> that served to reinforce reputation, reliability, and safety record.</p>}
                    {selectedIndustry === 'Retail & Grocery' && <p>Apart from select private label brands, Retail & Grocery represents the primary distribution channels for three other industries included in this sample, eclipsing all of them in <span className="font-text-bold emerald">product use</span> by double digits.</p>}
